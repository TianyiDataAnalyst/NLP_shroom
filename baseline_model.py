...
import wandb
#7b00f012eeda32b4607e0e21717d278bce245a1a
LABEL_LIST=[0,1]
...
# Load manually
    train_examples = []
    for path in data_files["train"]:
        train_examples.extend(load_jsonl(path))

    val_examples = load_jsonl(data_files["validation"])

    # Convert to Hugging Face Datasets manually
    dataset = DatasetDict({
    ...
    wandb.login(key="7b00f012eeda32b4607e0e21717d278bce245a1a")
...
def test_model(test_lang, model_path, data_path):
    ...
    test_path = f"{data_path}/mushroom.{test_lang}-tst.v1.jsonl"
